...
import { pullManifestsFromRegistry, getUrls } from "./getManifest.mjs"
...
async function downloadProcessLayers(manifests, layers) {
  ...
  for (const { layer, image_name, token } of processingLayers) {
    ...
    console.log(`image_name, layer`, image_name, layer);
    const { registryUrl, imageUrl, parsedImage } = await getUrls(image_name);
    console.log(`registryUrl, imageUrl, parsedImage`, registryUrl, imageUrl, parsedImage);
    const baseInPathSub = `${baseInPath}/images/${parsedImage.repository}`;
    console.log(`baseInPathSub`, baseInPathSub);
    ...
    try {
      ...
      const layerStream = await getBlob(imageUrl, await token, { layer: `sha256:${layer}` }, baseInPathSub)
