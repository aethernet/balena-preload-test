...
/**
 * Read supervisor version from file in the `in` folder
 * //TODO: This is only for dev purpose and should be removed as retrieving this file from s3 is already done in balena-img
 */
const getSupervisorVersion = async () => {
  return await fs.readFile(path.join(__dirname, "..", "in", "VERSION"), "utf-8")
}

/**
 * Read device arch from `device-type.json` in the `in` folder
 * //TODO: This is only for dev purpose and should be removed as retrieving this file from s3 is already done in balena-img
 */
const getArchForCurrentDt = async () => {
  const dtJson = await fs.readJson(path.join(__dirname, "..", "in", "device-type.json"))
  return dtJson.arch
}
...
const getSupervisorImageNameFor = async ({ version, arch, api, token }) => {
  const options = {
    ...
    url: `https://${
      api ?? process.API ?? "api.balena-cloud.com"
...
export { getSupervisorVersion, getArchForCurrentDt, getSupervisorImageNameFor }
