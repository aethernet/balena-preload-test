...
const generateFilesForLayer = ({ chain_id, diff_id, parent, lower, link, size, cache_id }) => {
  ...
  const files = [
    ...
    // emtpy `commited` file
    {
      header: { name: path.join(dockerOverlay2CacheId, "commited"), mode: "0o600" },
      content: "",
    },
    // emtpy `work` directory
    {
      header: { name: path.join(dockerOverlay2CacheId, "work"), mode: "0o777", type: "directory" },
    },
    // `link` file
    {
      header: { name: path.join(dockerOverlay2CacheId, "link"), mode: "0o644" },
      content: link,
    },
    // `diff` file
    {
      header: { name: path.join(dockerImageOverlay2LayerdbSha256ChainId, "diff"), mode: "0o755" },
      content: diff_id,
    },
    // `cache_id` file
    {
      header: { name: path.join(dockerImageOverlay2LayerdbSha256ChainId, "cache-id"), mode: "0o755" },
      content: cache_id,
    },
    // `size` file
    {
      header: { name: path.join(dockerImageOverlay2LayerdbSha256ChainId, "size"), mode: "0o755" },
      content: String(size),
    },
  ]
  ...
  if (parent)
    files.push({
      header: { name: path.join(dockerImageOverlay2LayerdbSha256ChainId, "parent"), mode: "0o755" },
      content: parent,
    })
  ...
  if (lower)
    files.push({
      header: { name: path.join(dockerOverlay2CacheId, "lower"), mode: "0o644" },
      content: lower,
    })
...
async function layerStreamProcessing({ layerStream, packStream, cache_id, injectPath }) {
  ...
  return new Promise((resolve) => {
    ...
    extract.on("entry", (header, stream, callback) => {
      ...
      const headerNewName = { ...header, name: path.join(injectPath, "docker", "overlay2", cache_id, "diff", header.name) }
