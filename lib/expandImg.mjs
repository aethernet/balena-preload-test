...
const getPartitions = async (image) => {
    ...
    const partitionsResults = {partitionArray: [], partitionArrayLength: 0};
    ...
    partitions.stdout.on('data', data => {
        ...
        partitionsResults.partitionArray = formatted;
...
const getImages = async () => {
    ...
    const partitions = await getPartitions(image);
    console.log('partitions', await partitions, 'partitionsLength', partitions?.length);
