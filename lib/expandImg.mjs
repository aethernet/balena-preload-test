...
/**
 * Scripted resize of partitions - This will make a 5Gb image - also in expander.sh
 * (size is set at step 3) 
 * (change value of 6 and 7 if you changed the size at 3):
 * 1. cp 77105551.img balenaos.img
 * 2. losetup /dev/loop10 balenaos.img
 * 3. fallocate -l 5120M balenaos.img 
 * -l length - Specifies the length of the range - fallocate preallocate or deallocate space to a file
 * 
 * 4. losetup -c /dev/loop10 
 * --c, --set-capacity loopdev - Force the loop driver to reread the size of the file associated with the specified loop device.
 * 
 * 5. parted -s -a opt /dev/loop10 "print free"
 * 6. parted -s -a opt /dev/loop10  "resizepart 4 5369" 
 * Do we need to do partition 4 or just the last partition?
 * 
 * 7. parted -s -a opt /dev/loop10  "resizepart 6 5369"
 * 8. parted -s -a opt /dev/loop10 "print free"
 * 9. `# partprobe -s /dev/loop10`
 * 10. `# resize2fs /dev/loop10p6`
 * 11. `# losetup -d /dev/loop10`
 * 12. `# zip balenaos.img.zip balenaos.img`
 * */ 
 ...
 * EXAMPLE WITH PIPE sudo dd if=/dev/sda bs=2048 count=44898303 conv=sync,noerror | pv -s 21G |sudo dd of=/dev/sdb
   Use fdisk to test

        obs=n    Set the output block size to n bytes instead of the default 512.

     of=file  Write output to file instead of the standard output.  Any regular output file is truncated unless the notrunc
              conversion value is specified.  If an initial portion of the output file is seeked past (see the oseek operand), the
     ...
     seek=n   Seek n blocks from the beginning of the output before copying.  On non-tape devices, an lseek(2) operation is used.
              Otherwise, existing blocks are read and the data discarded.  If the user does not have read permission for the tape,
              it is positioned using the tape ioctl(2) function calls.  If the seek operation is past the end of file, space from
              the current end of file to the specified offset is filled with blocks of NUL bytes.

     skip=n   Skip n blocks from the beginning of the input before copying.  On input which supports seeks, an lseek(2) operation
              is used.  Otherwise, input data is read and discarded.  For pipes, the correct number of bytes is read.  For all
              other devices, the correct number of blocks is read without distinguishing between a partial or complete block being
              read.


dd if=/dev/disk5 of=./images-expanded/tuckers.img bs=1024 conv=notrunc iflag=count_bytes, skip_bytes oflag=seek_bytes
dd if=/dev/disk5 of=./images-expanded/tuckers.img bs=1024 conv="notrunc" iflag="count_bytes, skip_bytes" oflag="seek_bytes"
dd: unknown iflag count_bytesdd if=/dev/disk5 of=./images-expanded/tuckers.img bs=4M conv="notrunc" bytes oflag="seek_bytes"
dd if=/dev/disk5 of=./images-expanded/tuckers.img bs=4M conv="notrunc" bytes oflag="seek_bytes"
dd if=/dev/disk5 of=./images-expanded/tuckers.img bs=4M conv="notrunc" oflag="seek_bytes"
dd if=/dev/disk5 of=./images-expanded/tuckers.img bs=4M conv="notrunc"
strace dd if=/dev/disk5 of=./images-expanded/fuckers.img bs=4M conv=notrunc
...
function createDDArgs(ifImage, ofImage, partitionInfos, sizeExpansion) {
    ...
    // TODO research whether these changes make it faster
...
const getFileSystem(device) {
    // -N     Don't execute, just show what would be done.
    const argsList = ['-N', device];
    // const argsList = [-N ${LOOP_DEV} |grep ${LOOP_DEV} | cut -d ']' -f 2 | cut -d ' ' -f 2 | cut -d '.' -f 2`]
    const result = await spawn('fsck', argsList, SPAWN_OPTS);
    const line = result.stdout.decode("utf8").strip().split("\n")[1]
    return line.rsplit(" ", 2)[-2].split(".")[1]
}

/** 
 * # 9. file system resizer resize partition 6 
...
const prepareImage = async (image) => {
    const { unzippedImg, unzippedDirImg } = unzipImage(img);
