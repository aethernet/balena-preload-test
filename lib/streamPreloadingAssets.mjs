  ...
  return new Promise((resolve) => {
    ...
    extract.on('entry', (header, stream, callback) => {
      ...
      if (hasHeaderPax){
         ...
         * It does exist in the layer as gstreamer
         ...
         *  }
        */
        logger.debug(`=> @layerStreamProcessing header ${inspect(header)}`)
        delete header.pax
      }
      const headerNewName = { ...header, name: path.join("docker", "overlay2", cache_id, "diff", header.name) }
      if (hasHeaderPax) {
        logger.debug(`=> @layerStreamProcessing headerNewName ${inspect(headerNewName)}`)
      }
...
const promisePacker = (pack) => (header, value, cb) =>
  new Promise((resolve, reject) => {
    logger.verbose(`=> pack header.name: ${header.name}`)
...
const downloadProcessLayers = async (manifests, layers, packStream, packFile) => {
  ...
  for (const key in processingLayers) {
    ...
    logger.info(`=> ${parseInt(key) + 1} / ${processingLayers.length} : sha256:${layer}`)
...
const packImageFiles = async (manifests, packFile) => {
  ...
  for (const key in manifests) {
    ...
    logger.info(`=> ${parseInt(key) + 1} : ${image_name}`)
    const imageIdName = image_id.split(':')[1]
    await packFile({ name: path.join(dockerImageOverlay2Imagedb, "content", "sha256", imageIdName), mode: "0o644" }, JSON.stringify(configManifestV2))
    await packFile(
      { name: path.join(dockerImageOverlay2Imagedb, "metadata", "sha256", imageIdName, "lastUpdated"), mode: "0o644" },
