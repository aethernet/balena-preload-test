...
import { mergeRepositories, repositoriesJsonInjectionPath } from "./repositoriesjson.mjs"
...
const streamPreloadingAssets = async ({ outputStream, user, password, app_id, release_id, balenaosRef, dataPartition = 6 }) => {
  ...
  const balenaOsRepositoriesJson = await getRepositoriesJsonFor(balenaosRef)
  const newRepositoriesJson = mergeRepositories({ manifests, originalRepositories: balenaOsRepositoriesJson })
  await packFile({ name: repositoriesJsonInjectionPath, mode: "0o644" }, JSON.stringify(newRepositoriesJson))
