...
import { getSupervisorImageNameFor, getSupervisorVersion, getArchForCurrentDt } from "./supervisor.mjs"
...
const streamPreloadingAssets = async ({ outputStream, user, password, app_id, release_id, balenaosRef, dataPartition = 6 }) => {
  ...
  const supervisorVersion = await getSupervisorVersion()
  const baseImages = [
    {
      image_name: await getSupervisorImageNameFor({
        version: supervisorVersion,
        arch: await getArchForCurrentDt(),
        api: process.env.API,
