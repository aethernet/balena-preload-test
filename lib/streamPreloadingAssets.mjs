...
async function getConfig(host, port, auth) {
  try {
    const fileName = 'config.json'
    const client = await Client({ host, port, ...auth,
      username: 'root',
      // password: 'password',
    ...
    console.error('==> getConfig error:', inspect(error, true, 2, true))
  }
}
...
const getAppsJson = async ({ app_id, release_id }, auth) => {
  ...
  const configJson = await getConfig(process.env.DEVICEADDRESS, 22222, auth)
