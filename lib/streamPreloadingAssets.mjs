...
const getAppsJson = async ({ app_id, release_id }, auth) => {
  if (featureFlags.apiApps) {
    // Get Config from API
    // Get fleetUuid from Config ?? Is the preloaded image going to be tied to a specific fleet?
    // Can't people just use it to make their own fleets?
    // Get release from API
    const config = await fs.readJson(`${baseInPath}/config.json`)
    const deviceId = config.uuid
    const options = {
      method: "GET",
      url: featureFlags.bob
        ? `https://api.${process.env.BALENAENV}/device/v3/${deviceId}/state` // bob's api endpoint
        : `https://api.${process.env.BALENAENV}/device/v3/fleet-state/${fleetUuid}/release/${releaseUuid}`, // @fisehara's version:
      headers: {
 ...
 */
async function layerStreamProcessing(layerStream, packStream, cache_id, compressedSize, layer, injectFolder) {
  ...
  return new Promise((resolve) => {
    ...
    extract.on("entry", (header, stream, callback) => {
      ...
      // change the name of the file to place it at the right position in tar archove folder tree
      const headerNewName = { ...header, name: path.join(injectFolder, "docker", "overlay2", cache_id, "diff", header.name) }
...
const promisePacker = (pack, injectFolder) => (header, value, cb) =>
  new Promise((resolve, reject) => {
    ...
    if (injectFolder) header.name = path.join(injectFolder, header.name)
 ...
 */
const downloadProcessLayers = async (manifests, layers, packStream, packFile, injectFolder) => {
  ...
  for (const key in processingLayers) {
    ...
    try {
      ...
      const { size, diff_id } = await layerStreamProcessing(layerStream, packStream, cache_id, compressedSize, layer, injectFolder)
...
const packImageFiles = async (manifests, packFile) => {
  ...
  for (const key in manifests) {
    ...
    await packFile(
      { name: path.join(dockerImageOverlay2Imagedb, "content", "sha256", shortImage_id), mode: "0o644" },
      JSON.stringify(configManifestV2)
    )
    await packFile(
 ...
 * @return {json} repositories.json
 */
const combineRepositories = async (repositoriesJson, repositories) => {
  // merge
...
const mergeRepositories = async (manifests, packFile, balenaosRef) => {
  ...
  // create all new repositories fragments
  const repositories = createAllRepositoriesFragments(manifests)

  // Combine the balenaos original repositories.json with the new repositories fragments
  const repositoriesNew = combineRepositories(repositoriesJson, repositories)

  // pack the output into the tar
  const repoPackFileName = path.join("docker", "image", "overlay2", "repositories.json")
  logger.info(`==> @mergeRepositories repoPackFileName ${repoPackFileName}`)
  await packFile({ name: repoPackFileName, mode: "0o644" }, JSON.stringify(repositoriesNew))
}

const streamBaseImage = ({ image: balenaosRef, packStream }) =>
  new Promise((resolve, reject) => {
    logger.warn("== Start streaming base image (balenaOs) @streamBaseImage ==")
    //TODO: this file should come from S3 (image-maker is already able to get this); for dev purpose we're taking it from fs instead

    const filePath = path.join(__dirname, "..", "in", `${balenaosRef}.img`)
    ...
    baseOsReadStream.on("open", function () {
      ...
      // beware that knowing the size in advance is required! If not specified we won't be able to add anything to the stream later on.
      baseOsReadStream.pipe(
        packStream.entry({
          name: `${balenaosRef}.img`,
          mode: "0o644",
          size,
        })
      )
 ...
 */
const streamPreloadingAssets = async ({ outputStream, user, password, app_id, release_id, balenaosRef, injectFolder }) => {
  // ##############
  // Processing
  // ##############
  ...
  const packFile = promisePacker(packStream, injectFolder) // this one is a promise
  const packManifest = promisePacker(packStream) // this one is a promise

  // 0. create and stream a manifest

  const manifest = {
    baseImage: balenaosRef,
    inject: [
      {
        partitionId: 5,
  ...
  await packManifest({ name: "manifest.json", mode: "0o777" }, JSON.stringify(manifest))

  // 0.1 start streaming the base image

  await streamBaseImage({ image: balenaosRef, packStream })
  ...
  logger.info("==> @streamPreloadingAssets manifests", inspect(manifests))

  // 5. create a `layers` array of object to keep track of what we're doing
  const layers = await getLayers(manifests)

  // // 8. download and process layers
  await downloadProcessLayers(manifests, layers, packStream, packFile, injectFolder)
