...
const streamPreloadingAssets = async ({ outputStream, user, password, app_id, release_id, balenaosRef, dataPartition = 6 }) => {
  ...
  const packFile = promisePacker(packStream, injectPath) // this one is a promise
  ...
  // 1.1  pack apps.json
  await packFile({ name: "apps.json", mode: "0o644"}, JSON.stringify(appsJson))
  ...
  // // 8. download and process layers
  await downloadProcessLayers(manifests, layers, packStream, packFile, injectPath)
