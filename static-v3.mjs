...
let token = await $`(cat < ~/.balena/token)`;
...
const baseInPath = `/tmp/in/images/${imageUrl.split("/").reverse()[0]}`;
...
for (const key in digests) {
  ...
  const digestChainIdFiles = [
    ...
    //size needs to be converted to string
    {name: `${baseOutPathChainId}/size`, val: `${size}`, mode: '0755'},
  ];
  makeFiles(digestChainIdFiles);
  if (key > 0) {
    fs.writeFileSync(`${baseOutPathChainId}/parent`, `sha256:${digests[key - 1].chainid}`, { mode: '0644' })
  }
  ...
  const digestCacheIdFiles = [
    ...
    {name: `${baseOutPathCacheId}/committed`, val: cacheid, mode: '0600'},
  ];
  makeFiles(digestCacheIdFiles);

  await $`tar -zxf ${path.join(baseInPath, `${gzipid}.tar`)} -C ${path.join(baseOutPath, "overlay2", cacheid, "diff")}`
  if(key > 0) {
    fs.writeFileSync(`${baseOutPathCacheId}/lower`, linkIdFullChain.slice(0, key).join(":"), { mode: '0644' })
  }
...
fs.writeFileSync(path.join(baseOutPath, '..', `${imageHash}.repositories.json`), JSON.stringify(repositories), { mode: '0644' })